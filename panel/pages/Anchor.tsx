      ...
      <Stack gap={1.2} sx={{ mt: 1.5 }}>
        {steps.map((step, i) => (
          <Stack key={i} direction="row" gap={1.2} alignItems="flex-start">
            ...
            <Typography variant="body2" color="text.secondary" sx={{ lineHeight: 1.6 }}>{step}</Typography>
          </Stack>
        ))}
      </Stack>
    </Box>
    ...
    <>
      ...
      <Section sx={{ py: { xs: 5, md: 7 } }}>
        <Stack direction="row" alignItems="center" gap={1.2} sx={{ mb: 1 }}>
          <Typography sx={{ fontFamily: "var(--font-arbeit-contrast)", fontWeight: 800, fontSize: { xs: 22, md: 26 }, letterSpacing: "-0.02em" }}>
            Canlı simülasyon
          </Typography>
          ...
          {!loadingLive && (
            <Chip
              size="small"
              label={live?.health?.ok ? "AYAKTA" : "CEVAP YOK"}
              sx={{
        ...
        <Typography variant="body2" color="text.secondary" sx={{ mb: 3, maxWidth: 660 }}>
          Aşağıdaki sayılar <b>{ANCHOR_HOME_DOMAIN}</b>'in şu an yayınladığı değerler — sayfa
          açılırken okundu. Banka ve KYC simüle, <b>Stellar ayağı gerçek</b>.
        </Typography>

        <Stack direction={{ xs: "column", md: "row" }} gap={2}>
          <Paper elevation={0} sx={{ flex: 1, p: 2.5, border: "1px solid", borderColor: "divider", borderRadius: 0 }}>
            <Typography variant="caption" color="text.secondary" fontWeight={700}>KUR (CANLI)</Typography>
            <Box sx={{ mt: 1.5 }}>
              <Reading
                label={`1 ${ANCHOR_ASSET_CODE} alış`}
                value={live?.rates.buy ? `${Number(live.rates.buy).toFixed(4)} ${FIAT_CODE}` : "—"}
                mono
              />
              <Reading
                label={`1 ${ANCHOR_ASSET_CODE} satış`}
            ...
            <Typography variant="caption" color="text.secondary" sx={{ display: "block", mt: 1.5, textTransform: "none", lineHeight: 1.6 }}>
              İki yön ayrı soruluyor; aradaki fark anchor'ın makasıdır ve tek bir kur
              göstermek onu gizlerdi.
            </Typography>
          </Paper>

          <Paper elevation={0} sx={{ flex: 1, p: 2.5, border: "1px solid", borderColor: "divider", borderRadius: 0 }}>
            <Typography variant="caption" color="text.secondary" fontWeight={700}>SERVİS</Typography>
            <Box sx={{ mt: 1.5 }}>
              <Reading label="Ortam" value={live?.health?.environment ?? "—"} />
              <Reading label="Stellar" value={live?.health?.stellarMode ?? "—"} />
              <Reading label="Ağ" value={live?.health?.networkPassphrase ?? "—"} mono />
              <Reading
                label="İhraççı"
        ...
        <Stack direction={{ xs: "column", md: "row" }} gap={4} sx={{ mt: 4 }}>
          <Steps title={`YÜKLEME · ${FIAT_CODE} → ${ANCHOR_ASSET_CODE}`} steps={ON_RAMP} />
          <Steps title={`ÇEKME · ${ANCHOR_ASSET_CODE} → ${FIAT_CODE}`} steps={OFF_RAMP} />
        </Stack>

        <Paper
          elevation={0}
          sx={{ mt: 3, p: 2.5, border: "1px dashed", borderColor: "divider", borderRadius: 0 }}
        >
          <Typography variant="body2" color="text.secondary" sx={{ lineHeight: 1.7 }}>
            Bu akışın ikisi de <strong>çalışıyor</strong> ve köprü sayfasından baştan sona
            denenebilir — havale simülasyonu dahil, zincire gerçek testnet {ANCHOR_ASSET_CODE}
            gidip gelerek.
          </Typography>
        </Paper>
      </Section>
