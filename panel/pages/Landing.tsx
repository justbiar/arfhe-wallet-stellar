import PrivacyStory from "../visuals/PrivacyStory";
...
function Section({ children, sx }: { children: React.ReactNode; sx?: object }) {
  return <Box sx={{ maxWidth: 1180, mx: "auto", px: { xs: 2, md: 3 }, ...sx }}>{pt(children)}</Box>;
}
...
function Flow({ title, steps, accent }: { title: string; steps: string[]; accent: string }) {
  return (
    <Paper elevation={0} sx={{ p: 2.5, border: "1px solid", borderColor: "divider", borderRadius: 3, height: "100%" }}>
      <Typography variant="caption" sx={{ color: accent, fontWeight: 700 }}>{pt(title)}</Typography>
      <Stack gap={1.2} sx={{ mt: 1.5 }}>
        {steps.map((s, i) => (
          <Stack key={i} direction="row" gap={1.2} alignItems="flex-start">
            ...
            <Typography variant="body2" color="text.secondary" sx={{ lineHeight: 1.55 }}>{pt(s)}</Typography>
          </Stack>
        ))}
      </Stack>
    </Paper>
...
export default function Landing() {
  ...
  return (
    <>
      <PrivacyStory />
      {/* ── Bridge introduction ── */}
      <Section sx={{ pt: { xs: 5, md: 7 }, pb: { xs: 5, md: 7 } }}>
        <Chip
          label={pt("STELLAR TESTNET · DEMO")}
          size="small"
          sx={{
        ...
        <Typography
          sx={{
            ...
            fontSize: { xs: 30, sm: 36, md: 44 }, lineHeight: 1.03, letterSpacing: "-0.03em", maxWidth: 880,
          }}
        >{pt(" Türk Lirası girer,")}<br />{pt("kendi cüzdanınızda USDC çıkar. ")}</Typography>

        <Typography variant="body1" color="text.secondary" sx={{ mt: 3, maxWidth: 620, fontSize: 17, lineHeight: 1.65 }}>{pt(" Bir tarafta tanıdık bir banka ekranı, diğer tarafta Arfhe Wallet. Aradaki yol Stellar'ın standart on/off-ramp protokolleri üzerinden kuruluyor — yani aynı entegrasyon, ileride gerçek bir kuruma da bağlanabiliyor. ")}</Typography>

        <Stack direction={{ xs: "column", sm: "row" }} gap={1.5} sx={{ mt: 4.5 }}>
          <Button
            component={Link} to="/bridge" variant="contained" size="large"
            endIcon={<ArrowForwardIcon />}
            sx={{ borderRadius: 3, px: 3, py: 1.4, fontSize: 14, fontWeight: 700 }}
          >{pt(" Köprüyü aç ")}</Button>
          <Button
            href={CHROME_STORE_URL} target="_blank" rel="noopener noreferrer"
            variant="outlined" size="large"
            sx={{ borderRadius: 3, px: 3, py: 1.4, fontSize: 14, fontWeight: 700, borderColor: "divider", color: "text.primary" }}
          >{pt(" Chrome'a ekle ")}</Button>
        </Stack>

        <Typography variant="caption" color="text.secondary" sx={{ display: "block", mt: 2.5, textTransform: "none" }}>{pt(`Arfhe Wallet ${FOUNDED_YEAR}'ten beri geliştiriliyor ve Chrome Web Mağazası'nda yayında.`)}{pt(" ")}
          <Box component={Link} to="/about" sx={{ color: "text.primary", textUnderlineOffset: 3 }}>{pt(" Cüzdan hakkında → ")}</Box>
        </Typography>
      </Section>
      ...
      <Box sx={{ borderTop: "1px solid", borderBottom: "1px solid", borderColor: "divider", py: { xs: 5, md: 7 } }}>
        <Section>
          <Stack direction="row" alignItems="center" gap={1.2} sx={{ mb: 3 }}>
            ...
            <Typography variant="caption" color="text.secondary" fontWeight={700}>{pt(" Köprü iki yönlü çalışır ")}</Typography>
          </Stack>

          <Box sx={{ display: "grid", gap: 2, gridTemplateColumns: { xs: "1fr", md: "1fr 1fr" } }}>
            <Flow
              title={pt("TRY → USDC  ·  YÜKLEME")}
              accent={accent}
              steps={[
      ...
      <Section sx={{ py: { xs: 5, md: 7 } }}>
        <Box sx={{ display: "grid", gap: { xs: 3, md: 5 }, gridTemplateColumns: { xs: "1fr", md: "1fr 1fr 1fr" } }}>
          <Box>
            <Typography variant="caption" color="text.secondary" fontWeight={700}>{pt("NE GERÇEK")}</Typography>
            <Typography variant="body2" color="text.secondary" sx={{ mt: 1, lineHeight: 1.65 }}>{pt(" Stellar ayağı gerçek. USDC gerçekten testnet üzerinde hareket ediyor, bakiyeler zincirden okunuyor, işlemler Horizon'da görülebiliyor. ")}</Typography>
          </Box>
          <Box>
            <Typography variant="caption" color="text.secondary" fontWeight={700}>{pt("NE SİMÜLE")}</Typography>
            <Typography variant="body2" color="text.secondary" sx={{ mt: 1, lineHeight: 1.65 }}>{pt(" Banka ve kimlik doğrulama. Gerçek para yok, gerçek IBAN yok, kişisel veri istenmiyor ve saklanmıyor. Havalenin geldiğini siz söylüyorsunuz. ")}</Typography>
          </Box>
          <Box>
        ...
        <Typography variant="caption" color="text.secondary" sx={{ display: "block", mt: 4, textTransform: "none" }}>{pt(" Karşı taraf: ")}<Box component="code" sx={{ fontFamily: "var(--font-arbeit-technik)" }}>{pt(ANCHOR_HOME_DOMAIN)}</Box>
        </Typography>
      </Section>
    </>
  );
